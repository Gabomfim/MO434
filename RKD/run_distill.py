...
from metric.utils import pdist, recall
...
def build_parser():
    ...
    parser.add_argument(
        "--log_confusion_matrix", choices=["true", "false"], default="true"
    )
...
def run_experiment(opts):
    ...
    def get_normalize(net):
        ...
        if isinstance(net, backbone.InceptionV1BN) or isinstance(
            net, backbone.GoogleNet
        ):
            return googlenorm
        return othernorm
    ...
    loader_train_sample = DataLoader(
        ...
        batch_sampler=NPairs(
            dataset_train, opts.batch, m=5, iter_per_epoch=opts.iter_per_epoch
        ),
        pin_memory=True,
        num_workers=8,
    )
    ...
    def _append_history_row(
        epoch, train_stats, train_rec, test_rec, best_train_rec, best_test_rec, lr
    ):
        row = [
            epoch,
    ...
    def train(loader, ep):
        ...
        for images, labels in train_iter:
            ...
            with torch.no_grad():
                _, t_b2, t_b3, t_b4, _, t_e = teacher(teacher_normalize(images), True)

            if isinstance(student.base, backbone.GoogleNet):
                assert (
                    opts.at_ratio == 0
                ), "AttentionTransfer cannot be applied on GoogleNet at current implementation."
                e = student(student_normalize(images))
                at_loss = torch.zeros(1, device=e.device)
            else:
                _, b2, b3, b4, _, e = student(student_normalize(images), True)
                at_loss = opts.at_ratio * (
                    at_criterion(b2, t_b2)
                    + at_criterion(b3, t_b3)
                    + at_criterion(b4, t_b4)
                )
            ...
            loss = (
                triplet_loss + dist_loss + angle_loss + quad_loss + dark_loss + at_loss
            )
            ...
            train_iter.set_description(
                "[Train][Epoch %d] Triplet: %.5f, Dist: %.5f, Angle: %.5f, Quad: %.5f, Dark: %5f, At: %5f"
                % (
                    ep,
                    triplet_loss.item(),
                    dist_loss.item(),
        ...
        print(
            "[Epoch %d] Loss: %.5f, Triplet: %.5f, Dist: %.5f, Angle: %.5f, Quad: %.5f, Dark: %.5f At: %.5f\n"
            % (
                ep,
                loss_mean,
                triplet_mean,
    ...
    def evaluate(net, normalize, loader, ep, return_embeddings=False):
        K = opts.recall
        net.eval()
    ...
    teacher_train_rec = evaluate(teacher, teacher_normalize, loader_train_eval, 0)
    teacher_val_rec = evaluate(teacher, teacher_normalize, loader_eval, 0)
    best_train_rec = evaluate(student, student_normalize, loader_train_eval, 0)
    best_val_rec, best_val_embeddings, best_val_labels = evaluate(
        student, student_normalize, loader_eval, 0, return_embeddings=True
    )
    ...
    for epoch in range(1, opts.epochs + 1):
        ...
        train_recall = evaluate(student, student_normalize, loader_train_eval, epoch)
        val_recall, val_embeddings, val_labels = evaluate(
            student, student_normalize, loader_eval, epoch, return_embeddings=True
        )
    ...
    _log_confusion_matrix(
        "best/test", best_val_embeddings, best_val_labels, step=opts.epochs
    )
    ...
    metrics_path = os.path.join(
        opts.save_dir if opts.save_dir is not None else ".", "distill_epoch_metrics.csv"
    )
