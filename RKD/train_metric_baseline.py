...
def run_experiment(opts):
    ...
    model = ConvNextMicro(num_classes=2, drop_path=opts.drop_path, dims=tuple(opts.dims),
                          depths=tuple(opts.depths), apply_softmax=False).to(device)
    ...
    art_name = "convnextmicro-metric-baseline-%s" % opts.dataset
    primary = opts.recall[0]

    tags = ["baseline", "metric", "convnextmicro", opts.dataset]
    if opts.wandb_tags:
    ...
    for epoch in range(start_epoch + 1, opts.epochs + 1):
        model.train()
        running = 0.0
        pbar = tqdm(loaders["train"], ncols=90, desc=f"[Baseline {epoch}]")
    ...
    final = evaluate_recall_splits(model, loaders, device, l2, opts.recall, tag="final ")
