...
% #############################################################################
% ##  SECTION LEFT BLANK ON PURPOSE --- THIS IS AN IMPLEMENTATION GUIDE.      ##
% ##                                                                          ##
% ##  Everything below in this section is a specification of the experiments  ##
% ##  to run and how to analyze them, NOT results. Fill each subsection with  ##
% ##  the actual tables/figures/text once the corresponding runs exist.       ##
% ##  Do NOT state a positive or negative headline until Phase 5 (multi-seed  ##
% ##  finals) is complete. The direction of every finding is genuinely        ##
% ##  unknown until the runs decide it.                                       ##
% #############################################################################

\emph{[This section is intentionally left blank. It is to be written from the
experiments specified below. The boxed guides in each subsection define the runs,
metrics, and analysis; they are instructions for producing the results, not the
results themselves. No headline outcome is claimed until the full protocol is run.]}
...
\emph{[Left blank. Produce here: per-dataset tables of test mAP@R / R-Precision /
Recall@K as \textbf{median (min--max)} over $\geq 5$ seeds (headline; $\geq 3$
elsewhere) for the five students on both teachers, plus a grouped mAP@R bar chart with
median bars and min--max whiskers. Analyze against H1 with the range-overlap rule
(non-overlapping seed ranges $=$ distinguishable) and the consistency-of-direction
rule above. Do not report mean\,$\pm$\,sem at $n{=}3$.]}
...
\emph{[Left blank. Produce here: the $\lambda_g$-viability plot (H0 gate); the
normalization ablation table/bars (H2); and the per-order characterization --- mAP@R,
convergence, stability, and cost as functions of $N \in \{3,4,8,16,17\}$ --- with an
explicit best-by-trade-off recommendation. $N=2$ is excluded by Lemma~\ref{lem:n2}.]}
...
\emph{[Left blank. Produce here: the profile-vs-MDS per-scenario table with the
stability and fidelity instrumentation (H3), and the regression-vs-contrastive
$\lambda_g$-robustness overlay with the width quantification and negative-quality
diagnostics (H4). Give scenario recommendations only where gaps show non-overlapping seed ranges
and are mechanistically explained.]}
...
\emph{[Left blank. Produce here: the matched-arity $N{=}3$ comparison (profile and MDS
vs RKD-A) per dataset/teacher, and its interpretation for whether an angular descriptor
is warranted.]}
...
\emph{[Left blank. Produce here: top-$k$ retrieval panels (successes and failures) for
Graph-RKD vs the strongest classic baseline, with the error-modes-not-ranking caveat.]}
