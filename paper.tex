...
\usepackage{graphicx}
\graphicspath{{figures/}{RKD/analysis/figures/}}  % also find the analysis result figures
\usepackage{booktabs}
\usepackage{array}
\usepackage{tabularx}
\usepackage{multirow}
...
detectable only by per-term logging. Deriving a rule that predicts the
quality-optimal $N$ from the minibatch size $K$ is outlined as future work.
% TODO: state the headline empirical outcome (whether Graph-RKD $N\geq3$ matches or
% beats RKD-D/RKD-A under the fair, re-tuned, multi-seed protocol) once the
% experiments are complete. Do not claim a positive or negative result until then.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/fig_tuples_linear.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.72\textwidth]{figures/fig_graphs_linear.pdf}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/fig_tuples_vs_graphs_log.pdf}
...
\begin{table}[h]
    ...
    \begin{tabularx}{\textwidth}{l X}
        ...
        Final runs & multi-seed headline ($\geq 3$ seeds), single-seed search phases (seeds not increased); report the \textbf{median} test mAP@R and the median-run checkpoint \\
        \bottomrule
    \end{tabularx}
\end{table}
...
% ##  mAP@R, regenerated by RKD/analysis notebooks. Cars-196 n=3 seeds;       ##
% ##  CUB-200 n=1 (seed 0 only, remaining seeds not run under the time        ##
% ##  budget) -> CUB cells are provisional (no significance claim). The guide ##
% ##  comments below each subsection are retained for reference.              ##
% #############################################################################
...
has $n{=}3$ seeds on \textbf{Cars-196} and $n{=}1$ (seed~0) on \textbf{CUB-200} --- the
two remaining CUB seeds were not run under the project's time budget, so all CUB
headline numbers are single-seed and should be read as \emph{provisional} (no seed
range, so we do not bold or claim significance on CUB cells). All figures/values are
...
\begin{itemize}\itemsep2pt
  ...
  \item[\textbf{I5}] \textbf{Multi-seed finals} ($\geq 5$ seeds for the headline
        comparison, $\geq 3$ elsewhere), reported as \textbf{median (min--max)} --- not
        mean\,$\pm$\,sem, which is not meaningful at $n{=}3$. No single-seed point
        estimates in the headline comparison.
...
\begin{table}[h]
    ...
    \caption{Headline (H1): median test mAP@R (\%) of the five students per cell.
             Cars-196 has $n{=}2$--$3$ seeds; CUB-200 has $n{=}1$ (seed~0 only; the
             remaining seeds were not run under the time budget, so CUB values are
             provisional and not bolded). Graph-RKD $=$ MDS, $N{=}4$, per-graph,
             $\lambda_g{=}0.01$. Best per Cars cell in \textbf{bold}.}
    \label{tab:headline}
    \begin{tabular}{lcccc}
        \toprule
        & Cars\,/\,R18 & Cars\,/\,CvT & CUB\,/\,R18 & CUB\,/\,CvT \\
        \midrule
        triplet-only        & 3.07 & 3.07 & 2.84 & 2.84 \\
        +\,RKD-D            & 1.79 & 2.21 & 3.36 & 2.97 \\
        +\,RKD-A            & 1.93 & 2.29 & 3.61 & 3.12 \\
        +\,RKD-D+RKD-A      & 1.76 & 1.93 & 3.40 & 3.10 \\
        +\,Graph-RKD        & \textbf{3.42} & \textbf{3.35} & 2.94 & 3.12 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[h]
    ...
    \caption{Median test mAP@R per student, one facet per (dataset, teacher) cell
             ...
             RKD variants are competitive. CUB is single-seed (provisional).}
    \label{fig:headline}
\end{figure}

The result is \textbf{dataset-dependent}. On \textbf{Cars-196}, Graph-RKD is the best
student on both teachers (3.42 / 3.35), and --- strikingly --- the classic RKD variants
...
regime inverts: classic RKD \emph{helps} (RKD-A best at 3.61 / 3.12, above the 2.84
floor), and Graph-RKD sits above the floor but at or below RKD-A --- it loses to RKD-A
on ResNet-18 (2.94 vs 3.61) and ties it on ConvNeXt-Tiny (3.12 vs 3.12). Thus Graph-RKD
...
single-seed and provisional, so we report them without a significance claim.
...
$\lambda_g{=}0.01$ Graph-RKD sits at/above the triplet-only floor, while every
$\lambda_g \geq 0.1$ falls below it and degrades monotonically. A longer-schedule
convergence check (80 epochs) confirmed the edge \emph{survives training} --- Graph-RKD
beat the floor by $\sim$14--18\% test mAP@R --- so H0 is accepted (marginally) and the
full grid uses $\lambda_g \in \{0.01, 0.1, 1\}$.
...
and both clearly beat \emph{none} (1.46); the \emph{hybrid} scheme, weakest in the
design-space probe, was dropped from the full run (Section~\ref{sec:setup}).
Figure~\ref{fig:h2} shows the ablation. We carry \emph{per-graph} forward as the
...
budget) median test mAP@R decreases mildly with $N$ (MDS: 1.48 / 1.34 / 1.20; profile:
1.33 / 1.28 / 1.13 at $N{=}3/4/8$), so low orders are the quality-by-cost sweet spot; we
adopt $N{=}4$ for the headline. Orders $N \in \{16,17\}$ were excluded because the MDS
...
executed orders (median test mAP@R at the 60-epoch search budget: MDS 1.48 / 1.34 / 1.20
vs profile 1.33 / 1.28 / 1.13 at $N{=}3/4/8$), with MDS marginally ahead at every $N$ ---
a gap well inside the run-to-run noise at these single-config points, so we do not claim
a winner on accuracy alone. The clean separation is on the \emph{mechanism}: the offline
...
\begin{table}[h]
    ...
    \caption{H5 --- matched-arity $N{=}3$: median test mAP@R (\%) of Graph-RKD (profile,
             MDS) vs RKD-A, per cell. Cars $n{=}2$--$3$; CUB $n{=}1$ (provisional).}
    \label{tab:h5}
    \begin{tabular}{lccc}
        \toprule
        cell & Graph-RKD (profile) & Graph-RKD (MDS) & RKD-A \\
        \midrule
        Cars / R18 & 3.39 & 3.53 & 1.93 \\
        Cars / CvT & 3.71 & 3.62 & 2.29 \\
        CUB / R18  & 3.26 & 2.86 & 3.61 \\
        CUB / CvT  & 3.00 & 2.99 & 3.12 \\
        \bottomrule
    \end{tabular}
\end{table}

The pattern mirrors the headline. On \textbf{Cars}, the distance descriptor at $N{=}3$
\emph{clearly beats} RKD-A (3.4--3.7 vs 1.9--2.3, both teachers): the triadic distance
structure already carries the useful signal there. On \textbf{CUB}, RKD-A is
competitive-to-better --- ahead on ResNet-18 (3.61 vs $\leq$3.26) and comparable on
ConvNeXt-Tiny (3.12 vs $\approx$3.0). So on CUB the \emph{angle} carries information the
distance-only descriptor misses, which is direct evidence for enriching the graph with
angular attributes (a hybrid distance-plus-angle descriptor) as future work
(Section~\ref{sec:conclusion}). CUB cells are single-seed and provisional.
...
same-class hits boxed --- are produced by notebook \texttt{05\_qualitative\_retrieval}
from the headline student checkpoint, contrasting Graph-RKD against the strongest classic
baseline per dataset (RKD-A on CUB, triplet-only on Cars, where classic RKD underperforms).
This notebook requires a GPU and the saved student checkpoints, so the panels are
generated on the training machine and inserted here. \emph{Caveat:} queries chosen by a
model's success/failure illustrate error \emph{modes}, not ranking quality --- the latter
is quantified by the tables above.
% \begin{figure}[h]\centering
%   \includegraphics[width=0.98\linewidth]{fig_qualitative.pdf}
%   \caption{Top-$k$ retrieval: Graph-RKD vs the strongest classic baseline. Green = same class.}
%   \label{fig:qual}
% \end{figure}
\emph{[Figura qualitativa a inserir: rodar \texttt{05\_qualitative\_retrieval} na máquina
com GPU e descomentar o bloco acima.]}
...
\emph{[Discussion of empirical findings to be completed once the experiments of
Section~\ref{sec:results} are available. Whether Graph-RKD at $N \geq 3$ matches or
exceeds the RKD-D/RKD-A baselines is left open until the fair, re-tuned, multi-seed
comparison decides it; no direction is presupposed here.]}
...
\emph{[Left blank. Add one short evidence-backed paragraph per hypothesis H1--H5 and
project-question Q1, using the analysis rules of Section~\ref{sec:results}. Preserve
the subsumption framing and the per-term-logging lesson above.]}
...
\emph{[The five project questions and their answers are to be finalized with the
results. In brief: Q1 (which teacher transfers best) depends on the new numbers;
Q2 targets the teacher's post-GAP $L_2$-normalized pooled embedding (a $192$-, $512$-,
and $768$-dimensional space for student and the two teachers, bridged with no
projection layer); Q3 is the $0.67$M-parameter student (a $16$--$43\times$ reduction);
Q4 pairs the triplet task loss with the relational graph loss --- the metric-learning
counterpart of the cross-entropy-plus-regression composition in the brief; and Q5 is
the adaptation of RKD~\cite{park2019relational} that constitutes the method. We note
that the project brief describes a classification setup, whereas we interpret the task
in RKD's native metric-learning setting; this reframes Q2--Q4 as above and is
licensed by Q5's invitation to adapt to~\cite{park2019relational}.]}

\section{Difficulties and Solutions}
\label{sec:difficulties}

% =============================================================================
% NOTE: dificuldades REAIS já encontradas + as inerentes ao método. Preencher/ajustar
% conforme os experimentos. Abaixo já estão as que sabemos.
% =============================================================================
...
\emph{[The empirical conclusion --- whether Graph-RKD at $N \geq 3$ matches or exceeds
the RKD-D/RKD-A baselines, and which normalization, descriptor, and objective are
preferable in which regime --- is to be stated once the experiments of
Section~\ref{sec:results} are complete. We do not presuppose a positive or negative
outcome.]}
